...
function createWindow(): void {
  ...
  win.loadFile(path.join(app.getAppPath(), "src", "renderer.html"));
}
...
ipcMain.handle("manual-review:apply", async (_event, payload) => {
  return applyManualReview(payload.reviewFilePath, payload.decisions, payload.dryRun ?? false);
});
...
ipcMain.handle("metadata:search", async (_event, payload) => {
  ...
  const results = await searchMetadata(payload.query, providers);
